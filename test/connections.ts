...
describe('connection fields', function () {
    ...
    function open_test(fields: any, verification: Function) {
        return function (done: Function) {
            ...
            container
                .connect(fields)
                .on('connection_open', function (context: rhea.EventContext) {})
        }
    }
    ...
    it(
        'single offered capability',
        open_test(
            { offered_capabilities: 'foo' },
            function (connection: rhea.Connection) {
                assert.equal(connection.remote.open.offered_capabilities, 'foo')
            }
        )
    )
    it(
        'multiple offered capabilities',
        open_test(
            { offered_capabilities: ['foo', 'bar'] },
            function (connection: rhea.Connection) {
                assert.equal(
                    connection.remote.open.offered_capabilities.length,
                    2
                )
                assert.equal(
                    connection.remote.open.offered_capabilities[0],
                    'foo'
                )
                assert.equal(
                    connection.remote.open.offered_capabilities[1],
                    'bar'
                )
            }
        )
    ...
    it(
        'multiple desired capabilities',
        open_test(
            { desired_capabilities: ['a', 'b', 'c'] },
            function (connection: rhea.Connection) {
                assert.equal(
                    connection.remote.open.desired_capabilities.length,
                    3
                )
                assert.equal(
                    connection.remote.open.desired_capabilities[0],
                    'a'
                )
                assert.equal(
                    connection.remote.open.desired_capabilities[1],
                    'b'
                )
                assert.equal(
                    connection.remote.open.desired_capabilities[2],
                    'c'
                )
            }
        )
    ...
    it(
        'channel max explicit',
        open_test({ channel_max: 10 }, function (connection: rhea.Connection) {
    ...
    it(
        ...
        open_test(
            ...
            function (connection: rhea.Connection) {
                assert.equal(
                    connection.remote.open.properties.flavour,
                    'vanilla'
                )
...
describe('connection error handling', function () {
    ...
    it('error handled', function (done: Function) {
        ...
        var c: rhea.Connection = rhea
            .create_container({ non_fatal_errors: [] })
...
describe('connection events', function () {
    ...
    beforeEach(function (done: Function) {
        ...
        container.on('connection_open', function (context: rhea.EventContext) {
            ...
            conn.local.open.offered_capabilities =
                conn.remote.open.desired_capabilities
        })
        listener = container.listen({ port: 0 })
        listener.on('listening', function () {
            done()
    ...
    it('dispatches events to correct handlers', function (done: Function) {
        ...
        var c1: rhea.Connection = container.connect({
            port: listener.address().port,
            desired_capabilities: 'one',
        })
        c1.on('connection_open', function (context: rhea.EventContext) {
            assert.equal(
                context.connection.remote.open.offered_capabilities,
                'one'
            )
            latch.decrement()
            context.connection.close()
        })
        var c2: rhea.Connection = container.connect({
            port: listener.address().port,
            desired_capabilities: 'two',
        })
        ...
        var c3: rhea.Connection = container.connect({
            port: listener.address().port,
...
describe('container id', function () {
    ...
    it('correctly sets desired container id', function (done: Function) {
        var container: rhea.Container = rhea.create_container({
            id: 'my-client-container',
        })

        var c1: rhea.Connection = container.connect(listener.address())
        c1.on('connection_open', function (context: rhea.EventContext) {
            assert.equal(
                context.connection.remote.open.container_id,
                'my-server-container'
            )
...
describe('link lookup and iteration', function () {
    ...
    it('finds sender or receiver', function (done: Function) {
        ...
        conn.on('connection_open', function () {
            assert.equal(
                conn.find_receiver(function (r: rhea.Receiver) {
                    return r.name === 'foo'
                }),
                r1
            )
            ...
            assert.equal(
                conn.find_sender(function (s: rhea.Sender) {
                    return s.name === 'oof'
                }),
                s1
            )
...
describe('default connect', function () {
    ...
    beforeEach(function (done: Function) {
        ...
        listener.on('listening', function () {
            ...
            fs.writeFile(
                filename,
                JSON.stringify({ port: listener.address().port }),
                'utf8',
                function () {
                    done()
                }
