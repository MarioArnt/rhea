...
describe("sasl plain", function () {
  ...
  it("successfully authenticates", function (done: Function) {
    container
      ...
      .on("connection_open", function (context) {
  ...
  it("handles authentication failure", function (done: Function) {
    container
      .connect({
        username: "whatsit",
        password: "anyoldrubbish",
        port: listener.address().port,
      })
      .on("connection_error", function (context) {
...
describe("sasl anonymous", function () {
  ...
  it("successfully authenticates", function (done: Function) {
    container
      .connect({ username: "bob", port: listener.address().port })
      .on("connection_open", function (context) {
        context.connection.close()
        done()
      })
  ...
  it("handles authentication failure", function (done: Function) {
    container
      ...
      .on("connection_error", function (context: rhea.EventContext) {
        var error = context.connection.get_error()
        assert.equal(
          (error as rhea.AmqpError).condition,
          "amqp:unauthorized-access"
        )
        assert.equal(
          (error as rhea.AmqpError).description,
          "No suitable mechanism; server supports ANONYMOUS"
        )
        done()
      })
  })
...
describe("user-provided sasl mechanism", function () {
  ...
  var testClientSaslMechanism = {
    ...
    step: function (challenge: any, callback: Function) {
      callback(null, "challengeResponse")
    },
  }
