...
describe('link fields', function () {
    ...
    for (var i = 0; i < link_types.length; i++) {
        ...
        it(
            t + ' name',
            open_test(t, { name: 'my-link' }, function (link: rhea.link) {
                assert.equal((link as any).remote.attach.name, 'my-link')
            })
        )
        it(
            'single offered ' + t + ' capability explicit',
            open_test(
                t,
                { offered_capabilities: 'foo' },
                function (link: rhea.link) {
                    assert.equal(
                        (link as any).remote.attach.offered_capabilities,
                        'foo'
                    )
                }
            )
        ...
        it(
            'single offered ' + t + ' capability aliased',
            open_test(
                t,
                { offered_capabilities: 'foo' },
                function (link: rhea.link) {
        ...
        it(
            'multiple offered ' + t + ' capabilities',
            open_test(
                t,
                { offered_capabilities: ['foo', 'bar'] },
                function (link: rhea.link) {
                    assert.equal(
                        (link as any).remote.attach.offered_capabilities.length,
                        2
                    )
                    assert.equal(
                        (link as any).remote.attach.offered_capabilities[0],
                        'foo'
                    )
                    assert.equal(
                        (link as any).remote.attach.offered_capabilities[1],
                        'bar'
                    )
                }
            )
        ...
        it(
            'single desired ' + t + ' capability',
            open_test(
                t,
                { desired_capabilities: 'foo' },
                function (link: rhea.link) {
                    assert.equal(
                        (link as any).remote.attach.desired_capabilities,
                        'foo'
                    )
                }
            )
        ...
        it(
            'multiple desired ' + t + ' capabilities',
            open_test(
                t,
                { desired_capabilities: ['a', 'b', 'c'] },
                function (link: rhea.link) {
                    assert.equal(
                        (link as any).remote.attach.desired_capabilities.length,
                        3
                    )
                    assert.equal(
                        (link as any).remote.attach.desired_capabilities[0],
                        'a'
                    )
                    assert.equal(
                        (link as any).remote.attach.desired_capabilities[1],
                        'b'
                    )
                    assert.equal(
                        (link as any).remote.attach.desired_capabilities[2],
                        'c'
                    )
                }
            )
        ...
        it(
            ...
            open_test(
                t,
                { properties: { flavour: 'vanilla', scoops: 2, cone: true } },
                function (link: rhea.link) {
                    assert.equal(
                        (link as any).remote.attach.properties.flavour,
                        'vanilla'
                    )
                    assert.equal(
                        (link as any).remote.attach.properties.scoops,
                        2
                    )
                    assert.equal(
                        (link as any).remote.attach.properties.cone,
                        true
                    )
                }
            )
    ...
    it(
        'source address as simple string',
        open_receiver_test('my-source', function (link: rhea.link) {
            assert.equal(
                (link as any).remote.attach.source.address,
                'my-source'
            )
        })
    )
    it(
        'source address aliased',
        open_receiver_test('my-source', function (link: rhea.link) {
            assert.equal(link.source.address, 'my-source')
        })
    )
    it(
        'source address as single nested value',
        open_receiver_test({ source: 'my-source' }, function (link: rhea.link) {
            assert.equal(
                (link as any).remote.attach.source.address,
                'my-source'
            )
        ...
        open_receiver_test(
            {
                source: {
                    ...
                    default_outcome: (
                        (amqp_messaging as any).modified() as any
                    ).described(),
                    outcomes: [
                        'amqp:list:accepted',
                        'amqp:list:rejected',
            ...
            function (link: rhea.link) {
                ...
                assert.equal(
                    (link as any).remote.attach.source.expiry_policy,
                    'session-end'
                )
                assert.equal((link as any).remote.attach.source.timeout, 33)
                assert.equal(
                    (link as any).remote.attach.source.distribution_mode,
                    'copy'
                )
                var descriptor = amqp_types.unwrap(
                    (link as any).remote.attach.source.filter['jms-selector']
                        .descriptor
                )
                ...
                assert.ok(
                    amqp_messaging.is_modified(
                        (link as any).remote.attach.source.default_outcome
                    )
                )
                assert.equal(
                    (link as any).remote.attach.source.outcomes.length,
                    4
                )
                assert.equal(
                    (link as any).remote.attach.source.outcomes[0],
                    'amqp:list:accepted'
                )
                assert.equal(
                    (link as any).remote.attach.source.outcomes[1],
                    'amqp:list:rejected'
                )
                assert.equal(
                    (link as any).remote.attach.source.outcomes[2],
                    'amqp:list:released'
                )
                assert.equal(
                    (link as any).remote.attach.source.outcomes[3],
                    'amqp:list:modified'
                )
                assert.equal(
                    (link as any).remote.attach.source.capabilities.length,
                    3
                )
                assert.equal(
                    (link as any).remote.attach.source.capabilities[0],
                    'a'
                )
                assert.equal(
                    (link as any).remote.attach.source.capabilities[1],
                    'b'
                )
                assert.equal(
                    (link as any).remote.attach.source.capabilities[2],
                    'c'
                )
            }
        )
    ...
    it(
        ...
        open_receiver_test(
            ...
            function (link: rhea.link) {
                ...
                assert.equal(
                    (link as any).remote.attach.source.capabilities,
                    'sourceable'
                )
            }
        )
        ...
        open_receiver_test(
            {
                source: {
                    dynamic: true,
                    dynamic_node_properties: { foo: 'bar' },
                },
            },
            function (link: rhea.link) {
                assert.equal((link as any).remote.attach.source.dynamic, true)
                assert.equal(
                    (link as any).remote.attach.source.dynamic_node_properties
                        .foo,
                    'bar'
                )
            }
    ...
    it(
        'target address as simple string',
        open_sender_test('my-target', function (link: rhea.link) {
            assert.equal(
                (link as any).remote.attach.target.address,
                'my-target'
            )
        })
    )
    it(
        'target address aliased',
        open_sender_test('my-target', function (link: rhea.link) {
            assert.equal(link.target.address, 'my-target')
        })
    )
    it(
        'target address as single nested value',
        open_sender_test({ target: 'my-target' }, function (link: rhea.link) {
            assert.equal(
                (link as any).remote.attach.target.address,
                'my-target'
            )
    ...
    it(
        ...
        open_receiver_test(
            ...
            function (link: rhea.link) {
                ...
                assert.equal(
                    (link as any).remote.attach.target.expiry_policy,
                    'connection-close'
                )
                assert.equal((link as any).remote.attach.target.timeout, 33)
                assert.equal(
                    (link as any).remote.attach.target.capabilities.length,
                    3
                )
                assert.equal(
                    (link as any).remote.attach.target.capabilities[0],
                    'd'
                )
                assert.equal(
                    (link as any).remote.attach.target.capabilities[1],
                    'e'
                )
                assert.equal(
                    (link as any).remote.attach.target.capabilities[2],
                    'f'
                )
            }
        )
    ...
    it(
        ...
        open_receiver_test(
            ...
            function (link: rhea.link) {
                ...
                assert.equal(
                    (link as any).remote.attach.target.capabilities,
                    'targetable'
                )
            }
        )
    ...
    it(
        ...
        open_receiver_test(
            ...
            function (link: rhea.link) {
                ...
                assert.equal(
                    (link as any).remote.attach.target.dynamic_node_properties
                        .foo,
                    'bar'
                )
            }
...
for (var local_role in roles) {
    describe(local_role + ' events', function () {
        ...
        beforeEach(function (done: Function) {
            ...
            container.on(roles[local_role] + '_open', function (context) {
                ...
                link.local.attach.offered_capabilities = (
                    link as any
                ).remote.attach.desired_capabilities
            })
            listener = container.listen({ port: 0 })
            listener.on('listening', function () {
                done()
        ...
        it('dispatches events to correct handlers', function (done: Function) {
            ...
            c.on(local_role + '_open', function (context) {
                assert.equal(
                    context[local_role].remote.attach.offered_capabilities,
                    'one'
                )
                latch.decrement()
                context[local_role].close()
            ...
            container
                ...
                ['open_' + local_role]({ desired_capabilities: 'three' })
    ...
    describe(local_role + ' error handling', function () {
        ...
        it('error handled', function (done: Function) {
            ...
            container.on(
                'connection_close',
                function (context: rhea.EventContext) {
                    assert.equal(error_handler_called, true)
                    done()
                }
            )
...
describe('settlement modes', function () {
    ...
    it('sender sends unsettled', function (done: Function) {
        ...
        client
            .connect(listener.address() as any)
            .attach_sender({ snd_settle_mode: 0 })
    })
    it('sender sends settled', function (done: Function) {
        ...
        client
            .connect(listener.address() as any)
            .attach_sender({ snd_settle_mode: 1 })
    })
    it('receiver requests send unsettled', function (done: Function) {
        server.on('sender_open', function (context) {
            ...
            context.sender.local.attach.snd_settle_mode =
                context.sender.snd_settle_mode
        })
        ...
        client
            .connect(listener.address() as any)
            .attach_receiver({ snd_settle_mode: 0 })
    })
    it('receiver requests send settled', function (done: Function) {
        ...
        client
            .connect(listener.address() as any)
            .attach_receiver({ snd_settle_mode: 1 })
    })
    it('receiver settles first', function (done: Function) {
        ...
        client
            .connect(listener.address() as any)
            .attach_receiver({ rcv_settle_mode: 0 })
    })
    it('receiver settles second', function (done: Function) {
        ...
        client
            .connect(listener.address() as any)
            .attach_receiver({ rcv_settle_mode: 1 })
    })
})
...
describe('preset sender options', function () {
    ...
    function connection_options_test(
        default_options: any,
        verification: any,
        open_options?: any
    ) {
    ...
    it(
        'properties',
        connection_options_test(
            { properties: { foo: 'bar' } },
            function (receiver: rhea.Receiver) {
    ...
    it('does not modify default options', function (done: Function) {
        ...
        c.open_sender({ target: 'foo' }).on('sender_open', function () {
            c.open_sender().on('sender_open', function () {
                c.close()
            })
        })
    })
