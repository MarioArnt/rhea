...
describe("link fields", function () {
  ...
  for (var i = 0; i < link_types.length; i++) {
    ...
    it(
      ...
      open_test(t, { offered_capabilities: "foo" }, function (link: rhea.link) {
    ...
    it(
      ...
      open_test(t, { desired_capabilities: "foo" }, function (link: rhea.link) {
    ...
    open_receiver_test(
      ...
      function (link: rhea.link) {
        ...
        var descriptor = amqp_types.unwrap(
          (link as any).remote.attach.source.filter["jms-selector"].descriptor
        )
  ...
  it(
    "dynamic source",
    open_receiver_test(
      { source: { dynamic: true, dynamic_node_properties: { foo: "bar" } } },
      function (link: rhea.link) {
        assert.equal((link as any).remote.attach.source.dynamic, true)
        assert.equal(
          (link as any).remote.attach.source.dynamic_node_properties.foo,
          "bar"
        )
      }
    )
  ...
  it(
    "dynamic target",
    open_receiver_test(
      { target: { dynamic: true, dynamic_node_properties: { foo: "bar" } } },
      function (link: rhea.link) {
        assert.equal((link as any).remote.attach.target.dynamic, true)
        assert.equal(
          (link as any).remote.attach.target.dynamic_node_properties.foo,
          "bar"
        )
      }
    )
