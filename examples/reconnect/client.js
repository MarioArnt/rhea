...
var args = require('../options.js')
    .options({
        request_interval: {
            describe: 'interval between requests',
            default: 1000,
        },
        ...
        idle_time_out: {
            describe:
                'maximum idle timeout; if nothing is received from peer for this interval, consider connection dead',
        },
        m: {
            alias: 'messages',
            default: 100,
...
var connect_options = {
    ...
    connection_details: function () {
        var details = {
            port: args.ports.length
                ? args.ports[attempt % args.ports.length]
                : args.ports,
            host: args.hosts.length
                ? args.hosts[attempt % args.hosts.length]
                : args.hosts,
        }
        attempt++
        return details
...
if (args.disable_reconnect) {
    ...
    if (args.fixed_delay) {
        ...
        if (args.initial_delay)
            connect_options.initial_reconnect_delay = args.initial_delay
    ...
    if (args.reconnect_limit)
        connect_options.reconnect_limit = args.reconnect_limit
}
