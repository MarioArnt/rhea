...
Expectations.prototype.complete = function (i) {
  ...
  if (
    this.expectations.every(function (o) {
      return o
    })
  ) {
    this.done()
...
Program.prototype.run = function (done) {
  var prog = this
  var name = this.name
  var p = child_process.fork(path.resolve(__dirname, this.name), this.args, {
    silent: true,
  })
  p.stdout.on("data", function (data) {
    prog.actual_output += data
    if (prog.triggers.length) {
      prog.triggers.forEach(function (t) {
        t.check()
      })
    }
  })
...
describe("brokered examples", function () {
  ...
  it("send and receive", function (done) {
    verify(done, [
      example("simple_recv.js").produces(
        times(100, function (i) {
          return '{"sequence":' + (i + 1) + "}"
        })
      ),
      example("simple_send.js").produces(
        times(100, function (i) {
          return "sent " + (i + 1)
  ...
  it("client/server", function (done) {
    ...
    var client_output = lines(
      requests.map(function (r) {
        return r + " => " + r.toUpperCase()
      })
    )
    var server_output = lines(
      requests.map(function (r) {
        return "Received: " + r
      })
    )
    while_running(done, [example("server.js").produces(server_output)]).verify([
      example("client.js").produces(client_output),
    ])
  })
  it("selector", function (done) {
    ...
    verify(done, [
      example("selector/recv.js", ["-m", "2"]).produces(red),
      example("selector/recv.js", [
        "-m",
        "1",
        "-s",
        "colour = 'blue'",
  ...
  it("pub-sub", function (done) {
    ...
    var pub_output = lines(
      messages.map(function (m) {
        return "sent " + m
      })
    )
    var sub_output = lines(["subscribed"].concat(messages.slice(0, 3)))
    var sub = example("durable_subscription/subscriber.js", [
      "-t",
      "topic://PRICE.STOCK.NYSE.RHT",
    ]).produces(sub_output)
    var pub = example("durable_subscription/publisher.js", messages).produces(
      pub_output
    )
  ...
  it("queue browser", function (done) {
    ...
    function browse(f) {
      verify(f, [
        example("queue_browser.js", ["-m", "4"]).produces(recv_output),
      ])
    }
    function consume(f) {
...
describe("direct examples", function () {
  ...
  it("send and receive", function (done) {
    verify(done, [
      example("direct_recv.js").produces(
        times(100, function (i) {
          return "{ sequence: " + (i + 1) + " }"
        })
      ),
      example("simple_send.js", ["-p", "8888"]).produces(
        times(100, function (i) {
          return "sent " + (i + 1)
        }) + "all messages confirmed\n"
      ),
  ...
  it("client/server", function (done) {
    ...
    while_running(done, [
      example("direct_server.js", ["-p", "8888"]).produces(server_output),
    ]).verify([example("client.js", ["-p", "8888"]).produces(client_output)])
  })
  it("tls connection", function (done) {
    while_running(done, [
      example("tls/tls_server.js", ["-p", "8888"]).produces(
        "Connected: TestClient [localhost]\n"
      ),
    ]).verify([
      example("tls/tls_client.js", ["-p", "8888", "-s", "localhost"]).produces(
        "Connected!\n"
      ),
    ])
  })
  ...
  it("sasl anonymous", function (done) {
    while_running(done, [
      example("sasl/sasl_anonymous_server.js", ["-p", "8888"]).produces(
        "Connected!\n"
      ),
    ]).verify([
      example("sasl/simple_sasl_client.js", [
        "-p",
        "8888",
        "--username",
        "anonymous",
  ...
  it("sasl plain", function (done) {
    while_running(done, [
      example("sasl/sasl_plain_server.js", ["-p", "8888"]).produces(
        lines(["Authenticating as bob", "Connected!"])
      ),
    ]).verify([
      example("sasl/simple_sasl_client.js", [
        "-p",
  ...
  it("websockets", function (done) {
    this.slow(1000)
    var output = times(100, function (i) {
      return "sent request-" + (i + 1) + "\nreceived request-" + (i + 1)
    })
    while_running(done, [example("websockets/echo.js")]).verify([
      example("websockets/client.js", ["-u", "ws:localhost:8888"]).produces(
        output
      ),
    ])
  })
  it("reconnect", function (done) {
    ...
    server.restart = true
    var client = example("reconnect/client.js", [
      "--request_interval",
      "5",
      "-m",
      "10",
