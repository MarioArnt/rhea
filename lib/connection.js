...
Connection.prototype._handle_error = function () {
  ...
  if (error) {
    ...
    handled =
      this.dispatch('connection_close', this._context({ error: error })) ||
      handled;
    ...
    if (!this._is_fatal(error.condition)) {
      ...
      this.dispatch(
        'error',
        new errors.ConnectionError(error.description, error.condition, this),
      );
    }
    return true;
  } else {
  ...
  if (
    ...
    if (!session)
      throw new errors.ProtocolError(
        'Invalid value for remote channel ' + frame.performative.remote_channel,
      );
  }
